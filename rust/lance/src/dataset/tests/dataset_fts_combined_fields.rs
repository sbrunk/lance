...
#[tokio::test]
async fn test_fts_combined_fields_tied_scores_are_deterministic() {
    // Neither scan orders ties on its own: `combined_fields_search` visits
    // candidates in ascending row-id order, so its top-k is reproducible, but
    // `ScoredDoc` compares on score alone, and once a flat scan is unioned in ties
    // would break by fragment arrival order. The `row_id ASC` second sort key is
    // what decides them, without which `limit`/`offset` pagination could skip and
    // repeat rows. This asserts that key's effect, not either scan's internals.
    let rows = 24usize;
    let (test_uri, _indexed) = indexed_two_column_dataset(&["aa", "aa"], &["aa", "aa"], None).await;
    // Every row is identical, so the correct answer is a full tie decided by row
    // id. Spread the appended rows over several fragments so the flat scan reads
    // them concurrently and arrival order is genuinely nondeterministic.
    let second = combined_fields_batch(
    ...
    // With every score tied, `(score DESC, row_id ASC)` pins the top-5 to the five
    // lowest ids, which asserts the specification rather than observing that
    // repeated runs happen to agree.
