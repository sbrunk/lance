...
impl FlatFieldStats {
    ...
    /// Fold one flat row's per-column contribution in.
    ///
    ...
    /// already count, so a row indexed for some target columns is not counted
    /// twice for them. It may admit an overlay-stale row, whose old value the
    /// index statistics then subtract (see [`CombinedFieldColumn::stale_rows`]).
    ...
    /// merging per-batch folds is exact and order-independent.
...
pub enum CombinedCorpusStats<'a> {
    ...
    ///
    /// For a legacy segment, which cannot name the rows an overlay made stale, so
    /// their pre-overlay values cannot be subtracted from its statistics.
...
pub async fn build_combined_bm25_scorer(
    ...
    if let Some(flat) = corpus.flat() {
        debug_assert_eq!(flat.doc_counts.len(), columns.len());
        debug_assert!(flat.doc_freqs.iter().all(|df| df.len() == terms.len()));
    }
        ...
        if corpus.folds_index_stats() {
            for index in &column.indices {
                ...
                let (total_tokens, num_docs, token_docs) = index
                    .bm25_row_stats_for_terms(&terms, column.stale_rows.as_deref(), metrics)
                    .await?;
